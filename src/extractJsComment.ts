...
fs.readdir(folderPath, (err, files) => {
	files.forEach((fileName) => {
		const inputFilePath = `./input/${fileName}`;
		...
		// const file2 = fs.readFileSync(`${intermediateOutputFilePath}.json`, { encoding: "utf-8" });
		// const jsondatatest = JSON.parse(file2)
		// const finalJson = extractTagSpecificData(inputFileName, jsondatatest);
	});
});
